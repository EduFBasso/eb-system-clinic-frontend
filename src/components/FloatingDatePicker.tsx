...
export default function FloatingDatePicker({
    ...
    React.useEffect(() => {
        if (open) {
            ...
            setView('day');
            setYearBase(
                Math.floor(selectedDate.getFullYear() / YEAR_RANGE) *
                    YEAR_RANGE,
            );
        }
    }, [open, selectedDate]);
                    ...
                    if (view === 'day') {
                        const canPrev =
                            ...
                            monthIndex(viewMonth) >
                                monthIndex(startOfMonth(minDay));
                        if (canPrev) setViewMonth(addMonths(viewMonth, -1));
                    } else if (view === 'month') {
                        setViewMonth(
                            v => new Date(v.getFullYear() - 1, v.getMonth(), 1),
                        );
                    } else {
    ...
    if (view === 'year') {
        ...
        return (
            <div
                ref={containerRef}
                style={containerStyle(clampedX, clampedY, drag.active)}
            >
                {dragHandle}
                <div
                    style={{
